...
public class MarsLaunch {
	...
	private boolean parseCommandArgs(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].toLowerCase().equals("we")) // added 14-July-2008
			{
				warningsAreErrors = true;
				continue;
			}
			if (args[i].toLowerCase().equals("sm")) // added 17-Dec-2009
			{
				startAtMain = true;
				continue;
			}
			if (args[i].toLowerCase().equals("smc")) // added 5-Jul-2013
			{
				selfModifyingCode = true;
				continue;
			}
			if (args[i].toLowerCase().equals("ic")) // added 19-Jul-2012
			{
			...
			// check for register name w/o $.  added 14-July-2008
			if (RegisterFile.getUserRegister("$" + args[i]) != null ||
					Coprocessor1.getRegister("$" + args[i]) != null) {
				registerDisplayList.add("$" + args[i]);
				continue;
